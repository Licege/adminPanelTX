    ...
    render() {
        ...
        return (
            <label className={visualType === 'button' ? ('Button' + (className ? ' '+className : '') + (disabled ? ' -disabled' : '')) : ("FileInput" + (className ? ' '+ className : '') +
                ...
                {
                    (visualType === 'input') &&
                    (value ? (type === 'application/pdf' ? <div>пдф доделай</div> : <img src={value} className='FileInput-Img'  alt=''/> )
                        : <span className="Input_Preview-Dish FileInput-Img" />)
                }
                <input ref={ this.refFileInput } id={id} type="file" {...options} onChange={this.onChange} className="FileInput-Field" disabled={disabled} />
